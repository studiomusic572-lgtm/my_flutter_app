...
class CategoryProductScreenState extends State<CategoryProductScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    // FIX: Use addPostFrameCallback to avoid "setState() called during build" error
    WidgetsBinding.instance.addPostFrameCallback((_) {
      // Fetch Subcategories
      categoryController.getSubCategoryList(widget.categoryID);

      // FORCE SET TO RESTAURANT MODE (Index 0)
      categoryController.setRestaurant(true);
            ...
            Expanded(child: NotificationListener(
              onNotification: (dynamic scrollNotification) {
                if (scrollNotification is ScrollEndNotification) {
                  // 2. Updated Controller Logic to match the new tab order
                  if((_tabController!.index == 0 && !catController.isRestaurant) || _tabController!.index == 1 && catController.isRestaurant) {
                    catController.setRestaurant(_tabController!.index == 0);
                    if(catController.isSearching) {
                      catController.searchData(
                      ...
                      if(_tabController!.index == 0) { // If Restaurant tab is selected (index 0)
                        catController.getCategoryRestaurantList(
                          catController.subCategoryIndex == 0 ? widget.categoryID
                              : catController.subCategoryList![catController.subCategoryIndex].id.toString(),
                          1, catController.type, false,
                        );
                      }else { // If Food tab is selected (index 1)
                        catController.getCategoryProductList(
                          catController.subCategoryIndex == 0 ? widget.categoryID
                              : catController.subCategoryList![catController.subCategoryIndex].id.toString(),
                          1, catController.type, false,
              ...
              child: TabBarView(
                ...
                // 3. Swapped Children Order: Restaurant content comes first
                children: [
                  // Index 0: Restaurant Content
